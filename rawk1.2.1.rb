...
class Rawk
  ...
  "Modified by Anthony Franco to work with latest Rails logging format\n" +
  ...
  def build_stats
    ...
    while @input.gets
      ...
      if @db_time
        ...
        time_string = $_[/((Rendering)|(View)): \d*\.?\d+/]
      else
        time_string = $_[/Completed in \d*\.?\d+/]
      end
      time_string = time_string[/\d*\.?\d+/] if time_string
      is_milliseconds = time_string.index(46) == nil if time_string # checks for '.' character
      time = time_string.to_f if time_string
      time /= 1000 if is_milliseconds
